...
local PRODUCT_KEY = "a1CmGG8XEwp"
local PRODUCE_SECRET = "fsBYVHUeOO5iH9R3"

--local PRODUCT_KEY = "a18utBDQZMg"
--local PRODUCE_SECRET = "bNssi05wg1yQk13Z"
...
local function getDeviceSecret()
    --默认使用设备的SN作为设备密钥，用户可以根据项目需求自行修改
    return misc.getSn()
    --return "L5OyxS6BS7EQPvQK8MHTuzp06Y9tfyYG"
...
local function rcvCbFnc(topic,qos,payload)
    ...
    if result and type(tjsondata)=="table" then
        local par = tjsondata["params"]
        if par ~= nil then
            --包含更新字段realtimeupdate
            if par["realtimeupdate"] == 1 then
                sys.publish("UPDATE_DTU", "RESTART")
            end
        end
    end
...
sys.taskInit(
    function()
        while true do
            local refcnt = 0
            res, cmd = sys.waitUntil("DLTCMD", nil)
            while true do
                flag = dlt.DltRunFlag()
                ...
                if flag == 0 then
                    ...
                    log.info("dltdata", dltdata)
                    local r = json.decode(dltdata)
                    local rv = r["com_data"] or "0"

                    sys.publish("DLTTASK", rv)
    ...
    function()
        while true do 
            ...
            if socket.isReady() then
                ...
                while true do
                    sys.waitUntil("UPDATE_DTU", 60000)
                    log.info("dtu", "update")

                    local res,data
                    sys.publish("DLTCMD","{\"cmd\":\"getdlt\",\"type\":\"addr\"}")
                    res, data = sys.waitUntil("DLTTASK", 5000)
                    if res == true then
                        _addr = tonumber(data)
                    end

                    sys.publish("DLTCMD", "{\"cmd\":\"getdlt\",\"type\":\"data\"}")
                    res, data = sys.waitUntil("DLTTASK", 5000)
                    if res == true then
                        _energy = tonumber(data)
                    end

                    sys.publish("DLTCMD", "{\"cmd\":\"getdlt\",\"type\":\"Voltage_A\"}")
                    res, data = sys.waitUntil("DLTTASK", 5000)
                    if res == true then
                        _ActiveEnergyImportInPhaseL1 = tonumber(data)
                    end

                    sys.publish("DLTCMD", "{\"cmd\":\"getdlt\",\"type\":\"Power_T\"}")
                    res, data = sys.waitUntil("DLTTASK", 5000)
                    if res == true then
                        _RealTimePower = tonumber(data)/10
                    end

                    log.info("aliYun", _addr, _energy, _ActiveEnergyImportInPhaseL1, _RealTimePower)
                    ...
                    local demo = {
                        ...
                        --addr = _addr
                            energy = _energy or 0,
                            ActiveEnergyImportInPhaseL1 = _ActiveEnergyImportInPhaseL1 or 0,
                            RealTimePower= _RealTimePower or 0,
                        },
                        method =  "thing.event.property.post"
                    }

                    local jsondemo = json.encode(demo)
                    aLiYun.publish("/sys/a1CmGG8XEwp/2gDTU/thing/event/property/post", jsondemo, 1)
                    --aLiYun.publish("/sys/a18utBDQZMg/6WSFIsLIdrvdKmgCvcoz/thing/event/property/post", jsondemo, 1)
                end
            end
        end
    end
