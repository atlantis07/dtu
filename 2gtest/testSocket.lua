...
local function MqttChannelTask(mqttconf)
    ...
    while true do
        while not _G["asyncClient"..id]:connect(address, port, "tcp") do 
            sys.wait(1000)
...
local function ChannelTask(id,prot,ping,keepalive,address,port,uid)
    ...
    while true do
        ...
        while not _G["asyncClient"..id]:connect(address, port) do
        ...
        --在connect的时候关闭
        --log.info("connect", "chstop flag 2")
        if type(chstopflag) == "string" then
            chstopflag = tonumber(chstopflag)
        end
...
function ChannelConfig(param)
    ...
    if result and type(tjsondata)=="table" then
        ...
        if status == "stop" then
            ...
            if not sys.waitUntil("CH_STOP", 5000) then
